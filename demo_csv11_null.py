...
for file in files:
    ...
    if file.split('.')[-1] in ['csv']:
        ...
        f = f2.astype(object).where(pd.notnull(f2), 'NULL')
        print(f)
        #print(f.dtypes)
        ...
        for value in values:
        # 5.3 构建插入数据的SQL语句
            #insert_sql = """upsert into YUANTONG_20191225 (order_id, "time", INFO, STATIONNAME, "TYPE") values({})""".format(str(value).replace('[','').replace(']','').replace('None',"NULL"))
            #print('insert_sql is:' + insert_sql)
            #print(str(value).replace('[','').replace(']','').replace('None',"NULL"))

            value = value + []
       #     print(str(tuple(value)+tuple(''))+',')
            #print(tuple(value))
            list_values.append(tuple(value))
            # tuple_value = (str(value).replace('[','').replace(']','').replace('None',"NULL"))
     #   print(list_values)
       # print(list_values)
        tuple_list_values=tuple(list_values)
     #   print(tuple_list_values)
        cursor.executemany('insert into YUANTONG_20191226 values(?,?,?,?,?)', tuple_list_values)
